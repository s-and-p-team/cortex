...
- **Allow-only rules (ALLOW/DENY model):** both Build and Rebuild source their rules from the PRB, which stays **allow-only** (every emitted `PolicyRule` relies on the `RuleEffect.ALLOW` default — see [`policy-rules-builder.md`](policy-rules-builder.md)). So a Rebuild currently re-asserts only grants; because `override=True` purges each role's deny edges too, any previously stored `Deny` rule for a rebuilt role is dropped and **not** re-emitted until deny extraction ships (an explicit follow-up).
